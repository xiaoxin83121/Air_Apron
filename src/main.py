...
def main(opt):
    ...
    torch.backends.cudnn.benchmark = not opt.not_cuda_benckmark and not opt.test
    Dataset = PascalVOC
    ...
    model = create_model(opt.arch, opt.heads, opt.heads_conv)
    ...
    trainer.set_device(opt.gpus, opt.chunk_size, opt.device)
