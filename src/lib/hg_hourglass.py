...
def make_layer_revr(k, inp_dim, out_dim, modules, layer=convolution, **kwargs):
    ...
    layers.append(layer(k, inp_dim, out_dim, **kwargs))  # in_dim是基准
    return nn.Sequential(*layers)
...
class exkp(nn.Module):
    def __init__(
        ...
        self.kps = nn.ModuleList([  # hourglass层
            kp_module(
        ...
        self.cnvs = nn.ModuleList([  # conv层
            make_cnv_layer(curr_dim, cnv_dim) for _ in range(nstack)
        ])

        self.inters = nn.ModuleList([  # residual层
            make_inter_layer(curr_dim) for _ in range(nstack - 1)
        ])

        self.inters_ = nn.ModuleList([  # 1x1卷积层
            nn.Sequential(
                nn.Conv2d(curr_dim, curr_dim, (1, 1), bias=False),
                nn.BatchNorm2d(curr_dim)
            ) for _ in range(nstack - 1)
        ])
        self.cnvs_ = nn.ModuleList([  # 1x1卷积层 dim不同
            nn.Sequential(
    ...
    def forward(self, image):  # 对应figure 4 right
        # print('image shape', image.shape)
        inter = self.pre(image)
        outs = []

        for ind in range(self.nstack):
            kp_, cnv_ = self.kps[ind], self.cnvs[ind]  # kps为一层kp_module，cnvs为一层卷积
            kp = kp_(inter)
            cnv = cnv_(kp)

            out = {}
            for head in self.heads:  # head里存放着不同的module
                layer = self.__getattr__(head)[ind]
                y = layer(cnv)
                out[head] = y

            outs.append(out)
            if ind < self.nstack - 1:  ## inter_+cnv_做Relu，再做一次残差
